...
namespace API.Controllers.v1
{
    ...
    public class UploadController : BaseController<DoanhNghiep, IDoanhNghiepBusiness>
    {
        ...
        [HttpPost]
        [Route("PostDoanhNghiepDichVuCASoChungThuByExcelFileAsync")]
        public virtual async Task<List<DoanhNghiepDichVuCA>> PostDoanhNghiepDichVuCASoChungThuByExcelFileAsync()
        {
            List<DoanhNghiepDichVuCA> result = new List<DoanhNghiepDichVuCA>();
            ...
            if (Request.Form.Files.Count > 0)
            {
                ...
                {
                    string fileExtension = Path.GetExtension(file.FileName);
                    string fileName = "DoanhNghiepDichVuCASoChungThu_" + GlobalHelper.InitializationDateTimeCode0001 + fileExtension;
                    var physicalPath = Path.Combine(_WebHostEnvironment.WebRootPath, GlobalHelper.Upload, fileName);
                    using (var stream = new FileStream(physicalPath, FileMode.Create))
                    {
                        file.CopyTo(stream);
                                    ...
                                    if (package.Workbook.Worksheets.Count > 0)
                                    {
                                        ...
                                        if (workSheet != null)
                                        {
                                            ...
                                            for (int i = 2; i <= totalRows; i++)
                                            {
                                                ...
                                                doanhNghiepDichVuCA = await _IDoanhNghiepDichVuCABusiness.GetBySoChungThuToAsync(doanhNghiepDichVuCA.SoChungThu);
                                                if (doanhNghiepDichVuCA != null)
                                                {
                                                    if (doanhNghiepDichVuCA.ID > 0)
                                                    {
                                                        if (workSheet.Cells[i, 2].Value != null)
                                                        {
                                                            doanhNghiepDichVuCA.HopDong = workSheet.Cells[i, 2].Value.ToString().Trim();
                                                        }
                                                        if (workSheet.Cells[i, 3].Value != null)
                                                        {
                                                            doanhNghiepDichVuCA.DonXinCapChungThuSo = workSheet.Cells[i, 3].Value.ToString().Trim();
                                                        }
                                                        if (workSheet.Cells[i, 4].Value != null)
                                                        {
                                                            doanhNghiepDichVuCA.CCCD = workSheet.Cells[i, 4].Value.ToString().Trim();
                                                        }
                                                        if (workSheet.Cells[i, 5].Value != null)
                                                        {
                                                            doanhNghiepDichVuCA.GiayPhepKinhDoanh = workSheet.Cells[i, 5].Value.ToString().Trim();
                                                        }
                                                        if (workSheet.Cells[i, 6].Value != null)
                                                        {
                                                            doanhNghiepDichVuCA.BienBanNghiemThu = workSheet.Cells[i, 6].Value.ToString().Trim();
                                                        }
                                                        if (workSheet.Cells[i, 7].Value != null)
                                                        {
                                                            doanhNghiepDichVuCA.HoaDon = workSheet.Cells[i, 7].Value.ToString().Trim();
                                                        }
                                                        await _IDoanhNghiepDichVuCABusiness.SaveAsync(doanhNghiepDichVuCA);
                                                        result.Add(doanhNghiepDichVuCA);
                                                    }
        ...
        [HttpPost]
        [Route("PostCACapBuByExcelFileAsync")]
        public virtual async Task<List<DoanhNghiepDichVuCA>> PostCACapBuByExcelFileAsync()
        {
            List<DoanhNghiepDichVuCA> result = new List<DoanhNghiepDichVuCA>();
            ...
            if (Request.Form.Files.Count > 0)
            {
                ...
                {
                    string fileExtension = Path.GetExtension(file.FileName);
                    string fileName = "SoChungThuCapBu_" + GlobalHelper.InitializationDateTimeCode0001 + fileExtension;
                    var physicalPath = Path.Combine(_WebHostEnvironment.WebRootPath, GlobalHelper.Upload, fileName);
                    using (var stream = new FileStream(physicalPath, FileMode.Create))
                    {
                        file.CopyTo(stream);
